...
def get_test(test_id: int, session: Session = Depends(get_session)):
    test = crud.get_test(session=session, test_id=test_id)
    if test is None:
...
def get_tests(skip: int = 0, limit: int = 10, session: Session = Depends(get_session)):
    tests = crud.get_tests(session=session, skip=skip, limit=limit)
    # if users is []:
    #     ...
    return tests

@app.delete("/tests/{test_id}", response_model=str)
def delete_test(test_id: int, session: Session = Depends(get_session)):
    test_id, test_name, user_id, err = crud.delete_test(session=session, test_id=test_id)
    if err is not None:
        raise HTTPException(status_code=404, detail=f"error: {err}")
...
@app.post("/tests/sats",response_model=schemas.SAT)
def post_sat(sat: schemas.SATCreate, session: Session = Depends(get_session)):
    sat, err = crud.post_sat(session=session, sat=sat)
    if err is not None:
        raise HTTPException(status_code=404, detail=f"unable to add sat: {err}")
    return sat

@app.get("/tests/sats/{sat_id}", response_model=schemas.Student)
def get_student(student_id: int,  request: Request, session: Session = Depends(get_session)):
    print(request.query_params)
    print(request.headers)
    student = crud.get_student(session=session, student_id=student_id)
